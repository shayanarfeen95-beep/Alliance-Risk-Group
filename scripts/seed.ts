...
import { getDb, closeDb, type Database } from '../lib/db/client';
...
const DEV_PASSWORD = 'westport2026';
...
const CONFIG = [
  {
    ...
    value: 'true',
    description:
      'Open item 1 — does ARG class its balance sheet in QBO? Determines whether DSO, DPO, CCC and Cash Runway are available by division or only at ARG Total (Defect 2).',
    isConfirmed: false,
  },
  {
    ...
    value: 'deal_property',
    description:
      'Open item 2 — how a HubSpot deal is attributed to a division: deal_property | pipeline | owner | none. When "none", sales and marketing KPIs report at ARG Total only. Do not invent an attribution rule.',
    isConfirmed: false,
  },
  {
  ...
  },
  {
    ...
    description:
      'Open item 4a — QBO accounts constituting marketing spend (CPL, ROAS, Marketing Efficiency Ratio). Requires Westport sign-off before these KPIs publish.',
    isConfirmed: false,
  },
  {
    ...
    description:
      'Open item 4b — QBO accounts constituting sales AND marketing spend (CAC numerator). A different set from 4a.',
    isConfirmed: false,
  },
  {
...
async function main() {
  ...
  // --- Reference data ------------------------------------------------------
  await db.insert(t.appConfig).values(CONFIG);
  ...
  console.log(`  divisions      ${DIVISION_SEED.length}`);
  console.log(`  periods        ${months.length} (closed through ${LAST_CLOSED_MONTH}, ${'2026-04-01'} open)`);
  console.log(`  accounts       ${ALL_ACCOUNTS.length}`);
  console.log(`  P&L rows       ${plCount}`);
  console.log(`  GL rows        ${dataset.gl.length}`);
  console.log(`  balance sheet  ${dataset.bs.length}`);
  console.log(`  budget rows    ${dataset.budget.length}`);
  console.log(`  deals          ${dataset.deals.length}`);
