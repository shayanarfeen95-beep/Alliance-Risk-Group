...
function secret(): Uint8Array {
  ...
  if (!value) {
    // Every instance must sign and verify with the same key. A per-process one
    // would mean a token minted by the instance that served the sign-in could
    // not be read by the instance that serves the next request, which is a
    // login page that never lets you past it.
